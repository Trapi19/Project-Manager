...
const ProjectEditor = ({ project, onSave, onBack, onCancelNew, isSaving, theme, onToggleTheme, onAddTimeEntry, onEditTimeEntry, onDeleteTimeEntry }) => {
    ...
    React.useEffect(() => {
        if (!hasChanges)
            setData(normalizeEditorProject(project));
    }, [project]);
    ...
    }))), React.createElement("label", { className: "block text-xs font-semibold text-gray-600 uppercase mb-1" }, "Logo del cliente"), React.createElement("div", { className: "project-edit-logo-row" }, React.createElement("div", { className: "project-edit-logo-preview" }, editLogoSrc && !editLogoFailed
        ? React.createElement("img", { src: editLogoSrc, alt: "Logo cliente", onError: () => setEditLogoFailed(true) })
        : React.createElement("span", null, getClientInitials(data))), React.createElement("div", { className: "project-edit-logo-controls" }, React.createElement("input", {
        type: "url",
...
const AlertsView = ({ projects, onBack }) => {
    ...
    const renderControlEmpty = (icon, title, text) => React.createElement("div", { className: "control-empty" }, React.createElement("i", { className: 'fas ' + icon }), React.createElement("strong", null, title), React.createElement("span", null, text));
    const renderAlertProject = (proj, tone, items, renderItem) => React.createElement("article", { key: proj.id, className: 'control-alert-card control-alert-card--' + tone }, React.createElement("div", { className: "control-alert-card-head" }, React.createElement("div", null, React.createElement("strong", null, proj.title || 'Proyecto sin titulo'), React.createElement("span", null, proj.client || 'Sin cliente')), React.createElement("button", { type: "button", onClick: () => openProject(proj.id) }, React.createElement("i", { className: "fas fa-arrow-up-right-from-square" }), "Ver proyecto")), React.createElement("div", { className: "control-alert-items" }, items.slice(0, 5).map(renderItem), items.length > 5 && React.createElement("div", { className: "control-more" }, '+' + (items.length - 5) + ' mas')));
    const renderControlSection = (className, icon, title, subtitle, count, content) => React.createElement("section", { className: 'control-panel ' + className }, React.createElement("div", { className: "control-panel-head" }, React.createElement("div", null, React.createElement("span", null, subtitle), React.createElement("h2", null, React.createElement("i", { className: 'fas ' + icon }), title)), React.createElement("em", null, count)), content);
    ...
    return (React.createElement("div", { className: "control-center-page wl-view-container" }, React.createElement("header", { className: "control-hero no-print" }, React.createElement("div", { className: "control-hero-main" }, React.createElement("button", { type: "button", onClick: onBack, className: "control-back" }, React.createElement("i", { className: "fas fa-arrow-left" }), "Volver"), React.createElement("div", null, React.createElement("span", { className: "control-eyebrow" }, "Alertas de cartera"), React.createElement("h1", null, "Centro de Control"), React.createElement("p", null, "Bloqueos, urgencias y proximos vencimientos de los proyectos."))), React.createElement("div", { className: "control-filters" }, React.createElement("label", { className: "control-search" }, React.createElement("i", { className: "fas fa-search" }), React.createElement("input", {
        type: "text",
        placeholder: "Buscar proyecto, cliente o PEP...",
        value: searchTerm,
        onChange: (e) => setSearchTerm(e.target.value)
    })), React.createElement("select", { value: clientFilter, onChange: (e) => setClientFilter(e.target.value) }, React.createElement("option", { value: "Todos" }, "Todos los clientes"), clients.map(c => React.createElement("option", { key: c, value: c }, c))))), React.createElement("section", { className: "control-kpi-grid" }, controlKpis.map(kpi => React.createElement("article", { key: kpi.label, className: 'control-kpi control-kpi--' + kpi.tone }, React.createElement("i", { className: 'fas ' + kpi.icon }), React.createElement("div", null, React.createElement("span", null, kpi.label), React.createElement("strong", null, kpi.value), React.createElement("small", null, kpi.helper))))), React.createElement("main", { className: "control-grid" }, renderControlSection("control-panel--blocked", "fa-lock", "Bloqueos por dependencias", "Tareas esperando a otra tarea", alertsData.blockedProjects.length, alertsData.blockedProjects.length
        ? React.createElement("div", { className: "control-card-list" }, alertsData.blockedProjects.map(proj => renderAlertProject(proj, "blocked", proj.items || [], t => React.createElement("div", { key: t.id, className: "control-alert-item" }, React.createElement("i", { className: "fas fa-lock" }), React.createElement("div", null, React.createElement("strong", null, t.tarea || 'Tarea sin titulo'), React.createElement("span", null, "Espera a: ", t.blockerName || 'Tarea pendiente'))))))
        : renderControlEmpty("fa-unlock", "Sin bloqueos activos", "No hay tareas bloqueadas por dependencias con los filtros actuales.")), renderControlSection("control-panel--urgent", "fa-triangle-exclamation", "Tareas urgentes", "Prioridad inmediata", alertsData.urgentProjects.length, alertsData.urgentProjects.length
        ? React.createElement("div", { className: "control-card-list" }, alertsData.urgentProjects.map(proj => renderAlertProject(proj, "urgent", proj.items || [], t => React.createElement("div", { key: t.id, className: "control-alert-item" }, React.createElement("i", { className: "fas fa-bolt" }), React.createElement("div", null, React.createElement("strong", null, t.tarea || 'Tarea sin titulo'), React.createElement("span", null, (t.area || 'General'), " - ", formatDue(t.fechaLimite)))))))
        : renderControlEmpty("fa-circle-check", "Sin tareas urgentes", "No hay tareas con prioridad urgente en los proyectos filtrados.")), renderControlSection("control-panel--upcoming", "fa-calendar-day", "Proximos vencimientos", "Agenda de los proximos 7 dias", alertsData.upcomingProjects.length, alertsData.upcomingProjects.length
...
const ChartsView = ({ projects, onBack }) => {
    ...
    React.useEffect(() => {
        ...
        if (analytics.assignees.length)
            pushChart(byAssigneeRef, { type: 'bar', data: { labels: analytics.assignees.map(row => row.name), datasets: [{ label: 'Pendientes', data: analytics.assignees.map(row => row.Pendiente), backgroundColor: '#ef4444', hoverBackgroundColor: '#dc2626', borderRadius: 0, borderSkipped: false, barPercentage: 0.72, categoryPercentage: 0.72 }, { label: 'En curso', data: analytics.assignees.map(row => row['En Curso']), backgroundColor: '#f59e0b', hoverBackgroundColor: '#d97706', borderRadius: 0, borderSkipped: false, barPercentage: 0.72, categoryPercentage: 0.72 }, { label: 'Completadas', data: analytics.assignees.map(row => row.Completado), backgroundColor: '#10b981', hoverBackgroundColor: '#059669', borderRadius: 0, borderSkipped: false, barPercentage: 0.72, categoryPercentage: 0.72 }] }, options: { ...commonOptions, plugins: { ...commonOptions.plugins, v4Premium: { preserveDatasetColors: true }, tooltip: { ...commonOptions.plugins.tooltip, callbacks: { label: (ctx) => `${ctx.dataset.label}: ${ctx.parsed.y || 0} tareas` } } }, scales: { x: { stacked: true, ticks: { color: theme.text }, grid: { display: false } }, y: { stacked: true, beginAtZero: true, ticks: { color: theme.text, precision: 0 }, grid: { color: theme.grid } } } } });
        const prioritySeries = seriesFromCounts(analytics.priorityCounts, ['Urgente', 'Alta', 'Media', 'Baja']);
