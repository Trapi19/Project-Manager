    ...
    return (React.createElement("div", { className: "home-dashboard" },
        React.createElement("section", { className: "home-hero" },
            React.createElement("div", { className: "home-hero-brand" },
                ...
                React.createElement("div", null,
                    React.createElement("h1", null, "Resumen ejecutivo"),
                    React.createElement("p", null, "Vista global de proyectos, carga de trabajo e incidencias."))),
        ...
        React.createElement("section", { className: `home-health home-health--${metrics.health.className}` },
            ...
            React.createElement("div", { className: "home-health-metrics" },
                React.createElement("span", null,
                    ...
                    " vencido",
                    metrics.overdueProjects === 1 ? '' : 's'),
                React.createElement("span", null,
                    metrics.totalIncidents,
                    " incidencia",
                    metrics.totalIncidents === 1 ? '' : 's'),
                React.createElement("span", null,
                    metrics.avgProgress,
                    "% avance"),
                React.createElement("span", null, highLoadLabel))),
        React.createElement("section", { className: "home-content-grid" },
            ...
            React.createElement("article", { className: "home-panel home-panel--workload" },
                ...
                metrics.workload.length ? (React.createElement("div", { className: "home-workload-list" }, metrics.workload.map(item => {
                    return (React.createElement("div", { className: `home-workload-row home-workload-row--${item.tone}`, key: item.name },
                        React.createElement("div", null,
...
const ProjectList = ({ projects, onCreate, onSelect, onDelete, onMoveProject, onBackup, onExportCSV, onImport, theme, onToggleTheme, storagePercent, statusFilter }) => {
    ...
    const handleProjectCardDrop = (e, targetProject) => {
        ...
        if (!draggedId || !onMoveProject)
            return;
        const targetEstado = normalizeProjectEstado((_g = targetProject === null || targetProject === void 0 ? void 0 : targetProject.meta) === null || _g === void 0 ? void 0 : _g.estado);
    ...
    const executiveSummary = (() => {
        ...
        nonCompletedProjects.forEach(p => {
            ...
            const title = ((_g = p.meta) === null || _g === void 0 ? void 0 : _g.titulo) || 'Proyecto';
            ...
            tasks.forEach(t => {
                ...
                if (est !== 'Completado' && String((t === null || t === void 0 ? void 0 : t.prioridad) || '').toLowerCase() === 'urgente') {
                    urgentTasks += 1;
                }
    ...
    }, React.createElement("div", { className: "exec-card-top" }, React.createElement("div", null, React.createElement("div", { className: "exec-label" }, "Incidencias"), React.createElement("div", {
...
const ProjectDetailDashboard = ({ project, onEdit, onAddTask, onAddTimeEntry, onEditTimeEntry, onDeleteTimeEntry, onPrint }) => {
    ...
    const renderSummary = () => {
        ...
        return React.createElement("div", { className: "project-summary-grid project-summary-grid--executive" },
            React.createElement("section", { className: `project-health-card project-health-card--${model.health.tone}` },
                ...
                React.createElement("div", { className: "project-health-chips" },
                    React.createElement("span", null,
                        ...
                        " tareas abiertas"),
                    React.createElement("span", null, model.incidentItems.length ? `${model.incidentItems.length} incidencias` : 'Sin incidencias'),
                    ...
                    React.createElement("span", null,
                        ...
                        " h imputadas"))),
            React.createElement("section", { className: "project-panel project-summary-next" },
                React.createElement("div", { className: "project-panel-head" },
                    React.createElement("div", null,
                        React.createElement("span", null, "Proximo hito"),
                        React.createElement("h2", null, "Vencimiento inmediato")),
                    React.createElement("button", { type: "button", onClick: () => setActiveTab('tasks') }, "Ver tareas")),
            ...
            React.createElement("section", { className: "project-panel project-summary-risks" },
                React.createElement("div", { className: "project-panel-head" },
                    React.createElement("div", null,
                        React.createElement("span", null, "Riesgos principales"),
                        React.createElement("h2", null, "Tareas criticas o bloqueadas")),
                    riskItems.length > 3 && React.createElement("button", { type: "button", onClick: () => setActiveTab('tasks') }, "Ver todas")),
            ...
            React.createElement("section", { className: "project-panel project-summary-status" },
                React.createElement("div", { className: "project-panel-head" },
                    React.createElement("div", null,
                        React.createElement("span", null, "Estado operativo"),
                        React.createElement("h2", null, "Documentacion e imputaciones"))),
                React.createElement("div", { className: "project-summary-mini-grid" },
                    React.createElement("button", { type: "button", onClick: () => setActiveTab('imputations') },
                    ...
                    React.createElement("button", { type: "button", onClick: () => window.location.hash = `#/wiki/${encodeURIComponent(String(project.id || ''))}` },
                        React.createElement("strong", null, model.wikiDocumented ? 'Wiki documentada' : 'Wiki pendiente'),
                        React.createElement("span", null, formatWikiDate(model.wiki.updatedAt))),
                    React.createElement("button", { type: "button", onClick: () => setActiveTab('incidents') },
                        React.createElement("strong", null, model.incidentItems.length ? `${model.incidentItems.length} incidencias` : 'Sin incidencias'),
                        React.createElement("span", null, "Ver detalle operativo")),
                    React.createElement("button", { type: "button", onClick: () => setActiveTab('documents') },
                        React.createElement("strong", null, hasDocuments ? 'Documentos vinculados' : 'Sin documentos'),
                        React.createElement("span", null, hasDocuments ? 'SharePoint disponible' : 'Sin enlaces')))),
            React.createElement("section", { className: "project-panel project-summary-action-panel" },
                React.createElement("div", { className: "project-panel-head" },
                    React.createElement("div", null,
                        React.createElement("span", null, "Siguiente accion"),
                        React.createElement("h2", null, "Recomendacion principal"))),
                React.createElement("div", { className: "project-summary-action" },
                    React.createElement("i", { className: "fas fa-lightbulb" }),
                    React.createElement("span", null, model.recommendations[0] || 'No hay acciones urgentes.'))));
    };
...
const ProjectEditor = ({ project, onSave, onBack, onCancelNew, isSaving, theme, onToggleTheme, onAddTimeEntry, onEditTimeEntry, onDeleteTimeEntry }) => {
    ...
    const updateTask = (id, field, value) => {
        ...
        setData(prev => {
            ...
            if (fromVal !== value) {
                ...
                nextProject = addActivityToProject(nextProject, `Tarea "${taskName}": ${label}: "${(fromVal !== null && fromVal !== void 0 ? fromVal : '')}" → "${(value !== null && value !== void 0 ? value : '')}"`, 'task');
            }
            return nextProject;
        });
        setHasChanges(true);
    ...
    const updateSubtask = (taskId, subId, newText) => {
        ...
        setData(prev => {
            const nextTasks = prev.tasks.map(t => {
                if (t.id !== taskId)
                    return t;
                const subs = (t.subtasks || []).map(s => s.id === subId ? { ...s, text: newText } : s);
    ...
    }))), React.createElement("label", { className: "block text-xs font-semibold text-gray-600 uppercase mb-1" }, "Logo del cliente"), React.createElement("div", { className: "project-edit-logo-row" }, React.createElement("div", { className: "project-edit-logo-preview" }, React.createElement("span", null, getClientInitials(data)), getClientLogoSrc(data) && React.createElement("img", { src: getClientLogoSrc(data), alt: "Logo cliente", onError: (e) => { e.currentTarget.style.display = 'none'; } })), React.createElement("div", { className: "project-edit-logo-controls" }, React.createElement("input", {
        type: "url",
    ...
    React.createElement("button", { onClick: () => deleteSubtask(task.id, sub.id), className: "btn-del-subtask", title: "Borrar subtarea" }, React.createElement("i", { className: "fas fa-trash" }))))), React.createElement("button", { onClick: () => addSubtask(task.id), className: "btn-add-subtask" }, React.createElement("i", { className: "fas fa-plus-circle" }), " Subtarea"))), React.createElement("td", { className: "px-6 py-4 min-w-[160px]" }, React.createElement("select", { className: `w-full border rounded text-sm p-1.5 outline-none font-medium ${task.estado === 'Completado' ? 'bg-emerald-50 text-emerald-700 border-emerald-200'
        ...
        } }, React.createElement("option", { value: "Pendiente" }, "Pendiente"), React.createElement("option", { value: "En Curso" }, "En Curso"), React.createElement("option", { value: "Completado" }, "Completado"))), React.createElement("td", { className: "px-6 py-4 min-w-[160px]" }, React.createElement("select", {
        className: `w-full border rounded text-sm p-1.5 outline-none font-medium ${(task.prioridad || 'Media') === 'Urgente' ? 'bg-rose-50 text-rose-700 border-rose-200'
    ...
    }, React.createElement("option", { value: "Urgente" }, "Urgente"), React.createElement("option", { value: "Alta" }, "Alta"), React.createElement("option", { value: "Media" }, "Media"), React.createElement("option", { value: "Baja" }, "Baja"))), React.createElement("td", { className: "px-6 py-4 min-w-[200px] internal-only" }, React.createElement("input", { type: "text", className: "w-full px-3 py-2 border border-gray-300 rounded-lg text-sm focus:ring-2 focus:ring-blue-500 focus:border-blue-500", value: task.asignadoA || '', onChange: (e) => updateTask(task.id, 'asignadoA', e.target.value), placeholder: "Asignado..." })), React.createElement("td", { className: "px-6 py-4 min-w-[280px]" }, React.createElement("textarea", { rows: "2", className: "w-full border border-gray-200 rounded text-xs p-2 focus:ring-1 focus:ring-blue-500 outline-none resize-y text-gray-600", value: task.detalles, onChange: (e) => updateTask(task.id, 'detalles', e.target.value), placeholder: "A\u00F1adir notas..." })), React.createElement("td", { className: "px-6 py-4 min-w-[180px]" }, React.createElement("input", { type: "date", className: "w-full border border-gray-200 rounded text-sm p-1.5 focus:ring-1 focus:ring-blue-500 outline-none text-center", value: toDateInputValue(task.fechaInicio), onChange: (e) => updateTask(task.id, 'fechaInicio', e.target.value) })), React.createElement("td", { className: "px-6 py-4 min-w-[180px]" }, React.createElement("input", { type: "date", className: "w-full border border-gray-200 rounded text-sm p-1.5 focus:ring-1 focus:ring-blue-500 outline-none text-center", value: toDateInputValue(task.fechaLimite), onChange: (e) => updateTask(task.id, 'fechaLimite', e.target.value) })), React.createElement("td", { className: "px-4 py-4 text-center align-middle" }, React.createElement("button", { onClick: () => deleteTask(task.id), className: "text-gray-300 hover:text-red-500 p-2 rounded transition-colors opacity-0 group-hover:opacity-100", title: "Eliminar" }, React.createElement("i", { className: "fas fa-trash" }))))))))))))));
};
...
const WorkloadView = ({ projects, onBack }) => {
    ...
    const workloadData = React.useMemo(() => {
        ...
        projects.forEach(p => {
            var _g;
            if (String((_g = p.meta) === null || _g === void 0 ? void 0 : _g.estado) === 'Completado')
                return;
            const tasks = (p.tasks || []);
            const idx = buildTaskIndex(tasks);
    ...
    // RENDERIZADO
    return (React.createElement("div", { className: "wl-view-container" }, React.createElement("div", { className: "wl-header-sticky no-print" }, React.createElement("button", { onClick: onBack, className: "btn-apple", style: { height: '36px', fontSize: '13px' } }, React.createElement("i", { className: "fas fa-arrow-left" }), "Volver"), React.createElement("div", { style: { width: '1px', height: '24px', background: 'var(--border)' } }), React.createElement("h2", { className: "wl-title" }, React.createElement("i", { className: "fas fa-users-cog", style: { color: 'var(--brand)' } }), "Carga de Trabajo")), React.createElement("div", { className: "max-w-7xl mx-auto p-6" }, workloadData.length === 0
        ? React.createElement("div", { style: { textAlign: 'center', padding: '60px 0', color: 'var(--muted)' } }, React.createElement("i", { className: "fas fa-check-circle", style: { fontSize: '40px', marginBottom: '16px', opacity: 0.5 } }), React.createElement("p", null, "No hay tareas pendientes asignadas."))
        : workloadData.map((person, idx) => (React.createElement("div", { key: idx, className: "wl-person-card" }, React.createElement("div", { className: "wl-person-header" }, React.createElement("div", { style: { display: 'flex', alignItems: 'center', gap: '16px' } }, React.createElement("div", { className: "wl-avatar-large" }, person.name.charAt(0).toUpperCase()), React.createElement("div", null, React.createElement("div", { className: "wl-person-name" }, person.name), React.createElement("div", { className: "wl-person-subtitle" }, person.totalTasks === 1 ? "1 tarea activa" : `${person.totalTasks} tareas activas`))), React.createElement("div", { className: `wl-load-badge ${person.totalTasks >= 5 ? 'high' : 'normal'}` }, person.totalTasks >= 5 ? "Carga Alta" : "Normal")), React.createElement("div", { className: "wl-grid-projects" }, person.projects.map(proj => (React.createElement("div", { key: proj.id, className: "wl-subcard", onClick: () => window.location.hash = `#/project/${proj.id}` }, React.createElement("div", { className: "wl-proj-title", title: proj.title }, proj.title), React.createElement("div", { className: "wl-client-tag" }, React.createElement("i", { className: "fas fa-building" }), proj.client), React.createElement("div", null, [...proj.tasks]
            .sort((a, b) => {
            const w = (x) => {
                const p = ((x === null || x === void 0 ? void 0 : x.prioridad) || 'Media').toString().toLowerCase();
                if (p === 'urgente')
                    return 0;
                if (p === 'alta')
                    return 1;
            ...
            if (wa !== wb)
                return wa - wb;
            const da = (a === null || a === void 0 ? void 0 : a.fechaLimite) ? new Date(a.fechaLimite).getTime() : Number.POSITIVE_INFINITY;
            const db = (b === null || b === void 0 ? void 0 : b.fechaLimite) ? new Date(b.fechaLimite).getTime() : Number.POSITIVE_INFINITY;
            return da - db;
        })
            .map(t => (React.createElement("div", { key: t.id, className: "wl-task-row" }, (() => {
        ...
        })(), React.createElement("div", { className: "wl-task-text" }, t.tarea, (() => {
...
const AlertsView = ({ projects, onBack }) => {
    ...
    const normClient = (p) => ((p.meta && p.meta.cliente) ? p.meta.cliente : 'Sin cliente').trim() || 'Sin cliente';
    const clients = React.useMemo(() => Array.from(new Set(projects.map(normClient))).sort((a, b) => a.localeCompare(b, 'es')), [projects]);
        ...
        filteredProjects.forEach(p => {
            ...
            if (hasOverdue || tooMany) {
                ...
                if (hasOverdue)
                    reasons.push("Tareas vencidas");
                if (tooMany)
                    reasons.push("Exceso de pendientes");
    ...
    React.createElement("div", null, React.createElement("h3", { className: "text-lg font-bold text-gray-800 mb-4 flex items-center gap-2" }, React.createElement("i", { className: "fas fa-triangle-exclamation text-amber-500" }), " Tareas Urgentes", React.createElement("span", { className: "alert-count-badge" }, alertsData.urgentProjects.length)), alertsData.urgentProjects.length === 0
        ...
        : React.createElement("div", { className: "grid grid-cols-1 md:grid-cols-2 gap-4" }, alertsData.urgentProjects.map(proj => React.createElement("div", {
            key: proj.id,
    ...
    React.createElement("div", null, React.createElement("h3", { className: "text-lg font-bold text-gray-800 mb-4 flex items-center gap-2" }, React.createElement("i", { className: "fas fa-calendar-day text-cyan-600" }), " Próximos Vencimientos (7 días)", React.createElement("span", { className: "bg-gray-100 text-gray-600 px-2 py-0.5 rounded-full text-xs" }, alertsData.upcomingProjects.length)), alertsData.upcomingProjects.length === 0
        ...
        : React.createElement("div", { className: "grid grid-cols-1 md:grid-cols-2 gap-4" }, alertsData.upcomingProjects.map(proj => (React.createElement("div", { key: proj.id, className: "wl-person-card", style: { padding: '16px', borderLeft: '4px solid #0891b2' }, onClick: () => window.location.hash = `#/project/${proj.id}` }, React.createElement("div", { className: "flex justify-between" }, React.createElement("div", null, React.createElement("div", { className: "font-bold text-gray-800" }, proj.title), React.createElement("div", { className: "text-xs text-gray-500 mb-2" }, proj.client)), React.createElement("div", { className: "text-xs font-bold text-cyan-700 bg-cyan-50 px-2 py-1 rounded h-fit" }, proj.items.length)), React.createElement("div", { className: "space-y-1" }, proj.items.map(t => React.createElement("div", { key: t.id, className: "flex justify-between text-xs bg-cyan-50/50 text-cyan-900 p-1.5 rounded" }, React.createElement("span", { className: "truncate mr-2 font-medium" }, t.tarea), React.createElement("span", { className: "whitespace-nowrap font-bold" }, window.formatFechaES(t.fechaLimite)))))))))))));
};
...
const ChartsView = ({ projects, onBack }) => {
    ...
    const openProject = (id) => { if (id != null)
        window.location.hash = '#/project/' + encodeURIComponent(String(id)); };
    const model = React.useMemo(() => {
        ...
        filteredProjects.forEach(project => {
            ...
            tasks.forEach(task => {
                ...
                if (done)
                    taskCounts.completada += 1;
                else if (blocked)
                    taskCounts.bloqueada += 1;
                else if (overdue)
                    taskCounts.vencida += 1;
                else if (effective === 'En Curso')
                    taskCounts.curso += 1;
                else
                    taskCounts.pendiente += 1;
                if (!done) {
                    openTasks += 1;
                    pOpen += 1;
            ...
            if (pInc > 0)
                incidentRows.push({ project, title, incidents: pInc, blocked: pBlocked, overdue: pOverdue });
            let health = 'Correcto';
            if (status === 'Completado')
                health = 'Completado';
            else if (status === 'En Pausa')
                health = 'En pausa';
            else if (pOverdue || pBlocked > 1)
                health = 'Critico';
            else if (pInc || pOpen > 0 && (stats.progress || 0) < 45)
                health = 'Atencion';
            healthRows.push({ project, title, status, health, progress: stats.progress || 0, open: pOpen, incidents: pInc, nextDue: ((_g = deadlineRows.filter(d => d.project === project).sort((a, b) => String(a.date).localeCompare(String(b.date)))[0]) === null || _g === void 0 ? void 0 : _g.date) || '' });
        });
    ...
    React.useEffect(() => {
        chartsRef.current.forEach(ch => { try {
            ch.destroy();
        }
        catch (e) { } });
        chartsRef.current = [];
        ...
        if (tasksRef.current && taskData.some(Boolean))
            chartsRef.current.push(new ChartJS(tasksRef.current, {
                type: 'doughnut',
        ...
        if (progressRef.current && model.progressRows.length)
            chartsRef.current.push(new ChartJS(progressRef.current, {
                type: 'bar',
        ...
        if (workloadRef.current && model.peopleRows.length)
            chartsRef.current.push(new ChartJS(workloadRef.current, {
                type: 'bar',
        ...
        if (imputationRef.current && model.imputationRows.length)
            chartsRef.current.push(new ChartJS(imputationRef.current, {
                type: 'bar',
        ...
        return () => { chartsRef.current.forEach(ch => { try {
            ch.destroy();
        }
        catch (e) { } }); chartsRef.current = []; };
    }, [model, themeTick]);
    const resetFilters = () => setFilters({ status: 'Todos', client: 'Todos', owner: 'Todos', month: new Date().toISOString().slice(0, 7) });
    const Empty = ({ icon, title, text }) => React.createElement("div", { className: "charts-empty" },
        React.createElement("i", { className: "fas " + icon }),
        React.createElement("strong", null, title),
        React.createElement("span", null, text));
    const ChartCard = ({ title, subtitle, children, wide }) => React.createElement("section", { className: "charts-card" + (wide ? ' charts-card--wide' : '') },
        React.createElement("div", { className: "charts-card-head" },
            React.createElement("div", null,
                React.createElement("span", null, subtitle),
                React.createElement("h2", null, title))),
        children);
    return React.createElement("div", { className: "charts-page" },
        React.createElement("header", { className: "charts-header" },
            React.createElement("button", { type: "button", className: "app-btn app-btn-ghost", onClick: onBack },
                React.createElement("i", { className: "fas fa-arrow-left" }),
                "Volver"),
            React.createElement("div", null,
                React.createElement("h1", null, "Analitica de proyectos"),
                React.createElement("p", null, "Seguimiento visual de progreso, carga, incidencias e imputaciones.")),
            React.createElement("div", { className: "charts-header-actions" },
                React.createElement("button", { type: "button", className: "app-btn app-btn-secondary", onClick: () => window.location.hash = '#/list' }, "Ver proyectos"))),
        React.createElement("section", { className: "charts-kpis" },
            React.createElement("article", null,
                React.createElement("i", { className: "fas fa-folder-open" }),
                React.createElement("div", null,
                    React.createElement("strong", null, model.projects.filter(p => getProjectStatus(p) !== 'Completado').length),
                    React.createElement("span", null, "Proyectos activos"))),
            React.createElement("article", null,
                React.createElement("i", { className: "fas fa-list-check" }),
                React.createElement("div", null,
                    React.createElement("strong", null, model.openTasks),
                    React.createElement("span", null, "Tareas abiertas"))),
            React.createElement("article", null,
                React.createElement("i", { className: "fas fa-shield-halved" }),
                React.createElement("div", null,
                    React.createElement("strong", null, model.incidentCount),
                    React.createElement("span", null, "Incidencias abiertas"))),
            React.createElement("article", null,
                React.createElement("i", { className: "fas fa-clock" }),
                React.createElement("div", null,
                    React.createElement("strong", null, model.hoursMonth.toLocaleString('es-ES')),
                    React.createElement("span", null, "Horas del mes"))),
            React.createElement("article", null,
                React.createElement("i", { className: "fas fa-route" }),
                React.createElement("div", null,
                    React.createElement("strong", null, model.kmMonth.toLocaleString('es-ES')),
                    React.createElement("span", null, "Kilometros del mes"))),
            React.createElement("article", null,
                ...
                React.createElement("div", null,
                    React.createElement("strong", null,
                        model.avgProgress,
                        "%"),
                    React.createElement("span", null, "Progreso medio")))),
        React.createElement("section", { className: "charts-filters" },
            React.createElement("label", null,
                "Estado",
            ...
            React.createElement("label", null,
                "Cliente",
                React.createElement("select", { value: filters.client, onChange: e => setFilters(f => ({ ...f, client: e.target.value })) },
                    React.createElement("option", null, "Todos"),
                    model.clients.map(c => React.createElement("option", { key: c }, c)))),
            React.createElement("label", null,
                "Responsable",
                React.createElement("select", { value: filters.owner, onChange: e => setFilters(f => ({ ...f, owner: e.target.value })) },
                    React.createElement("option", null, "Todos"),
                    model.owners.map(o => React.createElement("option", { key: o }, o)))),
            React.createElement("label", null,
                "Mes",
                React.createElement("input", { type: "month", value: filters.month, onChange: e => setFilters(f => ({ ...f, month: e.target.value })) })),
            React.createElement("button", { type: "button", onClick: resetFilters }, "Limpiar filtros")),
        model.projects.length === 0 ? React.createElement(Empty, { icon: "fa-filter-circle-xmark", title: "No hay proyectos con estos filtros", text: "Ajusta los filtros para volver a ver datos de cartera." }) : React.createElement(React.Fragment, null,
            React.createElement("main", { className: "charts-grid" },
                React.createElement(ChartCard, { title: "Estado de tareas", subtitle: "Distribucion global" }, model.totalTasks ? React.createElement("div", { className: "charts-canvas" },
                    React.createElement("canvas", { ref: tasksRef })) : React.createElement(Empty, { icon: "fa-chart-pie", title: "No hay tareas suficientes", text: "No hay tareas suficientes para generar este grafico." })),
                React.createElement(ChartCard, { title: "Progreso por proyecto", subtitle: "Top 10 ordenado por menor avance", wide: true }, model.progressRows.length ? React.createElement("div", { className: "charts-canvas" },
                    React.createElement("canvas", { ref: progressRef })) : React.createElement(Empty, { icon: "fa-chart-bar", title: "No hay proyectos", text: "No hay proyectos suficientes para calcular progreso." })),
                React.createElement(ChartCard, { title: "Carga de trabajo por persona", subtitle: "Tareas abiertas y horas", wide: true }, model.peopleRows.length ? React.createElement("div", { className: "charts-canvas" },
                    React.createElement("canvas", { ref: workloadRef })) : React.createElement(Empty, { icon: "fa-users", title: "No hay tareas asignadas", text: "Asigna tareas o imputa horas para visualizar carga por persona." })),
                React.createElement(ChartCard, { title: "Imputaciones por proyecto", subtitle: "Horas del periodo" }, model.imputationRows.length ? React.createElement("div", { className: "charts-canvas" },
                    React.createElement("canvas", { ref: imputationRef })) : React.createElement(Empty, { icon: "fa-clock", title: "Sin imputaciones", text: "No hay imputaciones registradas en el periodo." })),
                React.createElement(ChartCard, { title: "Incidencias por proyecto", subtitle: "Riesgo operativo" },
                    React.createElement("div", { className: "charts-rank-list" }, model.incidentRows.length ? model.incidentRows.map(row => React.createElement("button", { key: row.title, onClick: () => openProject(row.project.id) },
                        ...
                        React.createElement("span", null,
                            ...
                            " bloqueadas - ",
                            row.overdue,
                            " vencidas"))) : React.createElement(Empty, { icon: "fa-shield-heart", title: "Sin incidencias", text: "No hay incidencias registradas." }))),
                React.createElement(ChartCard, { title: "Proximos vencimientos", subtitle: "Maximo 5 hitos" },
                    React.createElement("div", { className: "charts-deadlines" }, model.deadlineRows.length ? model.deadlineRows.map((row, i) => React.createElement("button", { key: i, onClick: () => openProject(row.project.id) },
                        React.createElement("time", null, formatDate(row.date)),
                        React.createElement("div", null,
                            React.createElement("strong", null, row.task),
                            React.createElement("span", null,
                                row.projectTitle,
                                " - ",
                                row.owner,
                                " - ",
                                row.state)))) : React.createElement(Empty, { icon: "fa-calendar-check", title: "Sin vencimientos proximos", text: "No hay vencimientos proximos." })))),
            React.createElement("section", { className: "charts-health-table" },
                React.createElement("div", { className: "charts-card-head" },
                    React.createElement("div", null,
                        React.createElement("span", null, "Cartera"),
                        React.createElement("h2", null, "Salud de proyectos"))),
                React.createElement("div", { className: "charts-table-wrap" },
                    React.createElement("table", null,
                        React.createElement("thead", null,
                            React.createElement("tr", null,
                                React.createElement("th", null, "Proyecto"),
                                React.createElement("th", null, "Estado"),
                                React.createElement("th", null, "Salud"),
                                React.createElement("th", null, "Progreso"),
                                React.createElement("th", null, "Abiertas"),
                                React.createElement("th", null, "Incidencias"),
                                React.createElement("th", null, "Proximo vencimiento"))),
                        React.createElement("tbody", null, model.healthRows.map(row => React.createElement("tr", { key: row.title, onClick: () => openProject(row.project.id) },
                            React.createElement("td", null, row.title),
                            React.createElement("td", null, row.status),
                            React.createElement("td", null,
                                React.createElement("span", { className: 'charts-health-pill charts-health-pill--' + row.health.toLowerCase().replace(/\s+/g, '-') }, row.health)),
                            React.createElement("td", null,
                                row.progress,
                                "%"),
                            React.createElement("td", null, row.open),
                            React.createElement("td", null, row.incidents),
                            React.createElement("td", null, formatDate(row.nextDue))))))))));
};
...
const ProjectWiki = ({ project, onSave, onBack, isSaving }) => {
    ...
    (mode === 'view')
        ...
        }, React.createElement("i", { className: "fas fa-pen" }), React.createElement("span", { className: "hidden sm:inline" }, "Editar"))
        : React.createElement("button", {
            type: "button",
        ...
        ref: (el) => {
            ...
            setTimeout(() => {
                try {
                    ...
                    if (toolbar)
                        toolbar.style.display = (mode === 'view') ? 'none' : '';
                }
                catch (e) { }
            }, 0);
        },
...
const Sidebar = ({ view, projects, statusFilter, onNavigate, sidebarOpen, onClose, theme, onToggleTheme, onImport }) => {
    ...
    const isActive = (v, sf) => {
        if (sf !== undefined)
            return view === 'list' && statusFilter === sf;
        if (Array.isArray(v))
            return v.indexOf(view) !== -1;
        return view === v;
    };
    const ni = (icon, label, onClick, badge, active, indent) => React.createElement('button', {
        key: label,
...
const MainApp = () => {
    ...
    const loadProjectsLocal = async () => {
        try {
            ...
            if (!res.ok)
                throw new Error(`Error AWS: ${res.status}`);
        ...
        projects.forEach(p => {
            getProjectTimeEntries(p).forEach(e => {
                if (String(e.id) === String(id))
                    existingCreatedAt = e.createdAt || null;
            });
        });
        const entry = {
            id,
